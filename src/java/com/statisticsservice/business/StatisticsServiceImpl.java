...
public class StatisticsServiceImpl {
    ...
    public List<Certificate> getMostSearchedForCertificate(Integer max) {
        Connection connection = StatisticsDB.dbConnection();
        StatisticsQueries queries = new StatisticsQueries(connection);
        
        List<String> certificateCodes = null;
        try {
            certificateCodes = queries.getMostSearchedCertificates(max);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        
        List<Certificate> list = new ArrayList<Certificate>();
        for (String certificateCode : certificateCodes) {
            Certificate certificate = null;
            try {
                certificate = getCertificate(certificateCode);
            } catch (NonexistentCertificateException_Exception e) {
                ...
                certificate.setCertificateTitle("Unknown certificate code");
            }
            if (certificate != null) {
